...
const Profile = () => {
      ...
      if (token) {
        try {
          const response = await fetch(
            "http://localhost:8080/guanago/usuarios/datos-usuario",
            {
              method: "GET",
              headers: {
      ...
      const response = await fetch(
        `https://translation.googleapis.com/language/translate/v2`,
        {
          ...
          headers: {
            ...
            Authorization:
              "Bearer " + "AIzaSyA2-q3jpNjbRR0dW9tGJICK5mYhMqEeZZo", // Secure this properly
          },
          body: JSON.stringify({
            q: text,
            target: targetLang,
  ...
  const handleUpdateProfile = async (event) => {
    ...
    if (token) {
      try {
        const response = await fetch(
          "http://localhost:8080/guanago/usuarios/editar-usuario",
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    if (result.isConfirmed) {
      ...
      if (token) {
        try {
          ...
          const response = await fetch(
            "http://localhost:8080/guanago/usuarios/editar-contrasena",
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
        ...
        Swal.fire("Error", "No se encontró un token de sesión.", "error");
      }
    }
  };
  ...
  return (
    <div className="profile-container">
      <aside className="sidebar">
        ...
        <ul>
          <li onClick={() => changeSection("datos-personales")}>
            {textos[currentLanguage].datosPersonales}
          </li>
          <li onClick={() => changeSection("preferencias")}>
            {textos[currentLanguage].preferencias}
          </li>
          <li onClick={() => changeSection("datos-de-pago")}>
            {textos[currentLanguage].datosDePago}
          </li>
        </ul>
      </aside>
            ...
            <form onSubmit={handleUpdateProfile}>
              ...
              <div className="input-group">
                <label htmlFor="apellido">
                  {textos[currentLanguage].apellido}
                </label>
                <input
                  type="text"
                  id="apellido"
              ...
              <div className="input-group">
                <label htmlFor="correo">
                  {textos[currentLanguage].correoElectronico}
                </label>
                <input
                  type="email"
                  id="correo"
            ...
            <form onSubmit={handleChangePassword}>
              <div className="input-group">
                <label htmlFor="contrasena">
                  {textos[currentLanguage].nuevaContrasena}
                </label>
                <input
                  type="password"
                  id="contrasena"
              ...
              <div className="button-group">
                <button type="submit">
                  {textos[currentLanguage].cambiarContrasena}
                </button>
              </div>
            </form>
          </div>
              ...
              <div className="preferencia-item">
                ...
                <div className="preferencia-info">
                  <button className="btnSecondary">
                    {textos[currentLanguage].actualizar}
                  </button>
                </div>
              </div>
              <div className="preferencia-item">
            ...
            <div className="datos-pago-container">
              ...
              <div className="metodos-pago">
                ...
                <button className="btn">
                  {textos[currentLanguage].añadirTarjeta}
                </button>
              </div>
            </div>
          </div>
